...
#                 if not frame_data:
#                     logger.warning("Received empty frame data")
#                     await websocket.send_json({"status": "error", "error": "Empty frame data received"})
#                     continue
...
#                     await websocket.send_json({"status": "success", "counts": counts, "detections": detections})

#                 except ValueError as ve:
#                     logger.error(f"Frame processing error: {str(ve)}")
#                     await websocket.send_json({"status": "error", "error": str(ve)})
#                 except Exception as e:
#                     logger.error(f"Unexpected error during frame processing: {str(e)}")
#                     await websocket.send_json({"status": "error", "error": "Internal processing error"})
...
#             except Exception as e:
#                 logger.error(f"WebSocket communication error: {str(e)}")
#                 await websocket.send_json({"status": "error", "error": "Communication error"})

#     except Exception as e:
#         logger.error(f"WebSocket connection error: {str(e)}")
#     finally:
#         if websocket.client_state == "CONNECTED":  # Ensure it's not already closed
